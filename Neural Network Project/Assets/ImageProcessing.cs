...
public class ImageManipualtion : MonoBehaviour
{
    [SerializeField] Texture2D image;
    [SerializeField] Texture2D manipImage;
    ...
    [SerializeField] Image before;
    [SerializeField] Image after;
    // Start is called before the first frame update
    void Start()
    {
        reload.onClick.AddListener(recreate);
    }
    ...
    public void recreate()
    {
        ...
        manipImage = ApplyScale(manipImage, scale);

        manipImage = ApplyOffset(manipImage, offsetX, offsetY);

        //manipImage = ApplyRotation(image, angle);

        //  manipImage = ApplyNoise(manipImage);

        //Apply all changes

        //Paste images

        before.sprite = Sprite.Create(image, new Rect(new Vector2(0, 0), new Vector2(image.width, image.height)), new Vector2(0, 0));

        after.sprite = Sprite.Create(manipImage, new Rect(new Vector2(0, 0), new Vector2(manipImage.width, manipImage.height)), new Vector2(0, 0));

        //Save Texture as PNG
        byte[] bytes = manipImage.EncodeToPNG();


        File.WriteAllBytes("Assets/New Images/Testing/" + "idk" + ".png", bytes);
    }
